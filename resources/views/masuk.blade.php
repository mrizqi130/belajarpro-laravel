        ...
        <div class="col-4 mt-5 rounded card-bel">
            <form class="form-floating needs-validation" action="{{route('submit')}}" method="POST">
                ...
                <div class="mb-3 ms-3 me-3">
                    ...
                    <input type="email" class="form-control @if(session('login-failed')) <?= "is-invalid" ?> @endif)" id="email" name="email" aria-describedby="email" required placeholder="nama@gmail.com" value="@if($data['remember'])<?= $data['email'] ?>@endif @if(session('login-failed'))<?="session('login-failed.email')" ?>@endif">
